 ...
 */
export async function findMatchingIntents(providerId: string): Promise<MatchResult[]> {
  const provider = await providerStore.get(providerId);
  if (!provider) return [];

  // Get all active intents
  const activeIntents: Intent[] = await intentStore.list({ status: 'active' });
...
export async function findMatchingProviders(
  intentId: string
): Promise<Array<{ provider: Provider; score: number; matchedCapabilities: string[] }>> {
  const intent: Intent | undefined = await intentStore.get(intentId);
  if (!intent) return [];

  // Get all online providers
  const onlineProviders: Provider[] = await providerStore.list({ status: 'online' });
 ...
 */
function getMatchedCapabilities(intent: Intent, _provider: Provider, providerCats: string[]): string[] {
  ...
  return providerCats.filter((cat) => {
    ...
    return catLower === intentCategory || catLower.includes(intentCategory) || intentCategory.includes(catLower);
  });
}
...
export async function getMatchStats() {
  ...
  for (const intent of activeIntents) {
    const matches = await findMatchingProviders(intent.id);
  ...
  return {
    ...
    avgMatchesPerIntent: activeIntents.length > 0 ? (totalPossibleMatches / activeIntents.length).toFixed(2) : '0',
  };
}
