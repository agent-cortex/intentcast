...
import { acceptOfferBodySchema, submitOfferBodySchema } from '../schemas/offer.js';
...
router.post(
  '/intents/:id/offers',
  validateBody(submitOfferBodySchema),
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (parseFloat(priceUsdc) > parseFloat(intent.maxPriceUsdc)) {
      throw new AppError({
        code: 'BAD_REQUEST',
        statusCode: 400,
        message: 'Price exceeds max',
        details: { maxPriceUsdc: intent.maxPriceUsdc, offeredPrice: priceUsdc },
      });
    }
    ...
    if (existingOffer) {
      throw new AppError({
        code: 'CONFLICT',
        statusCode: 409,
        message: 'Provider already submitted an offer',
        details: { existingOfferId: existingOffer.id },
      });
    }
...
router.post(
  '/intents/:id/accept',
  validateBody(acceptOfferBodySchema),
  asyncHandler(async (req: Request, res: Response) => {
    const { id: intentId } = req.params;
    ...
    if (offer.intentId !== intentId) {
      throw new AppError({
        code: 'BAD_REQUEST',
        statusCode: 400,
        message: 'Offer does not belong to this intent',
        details: { offerId, intentId },
      });
    }
    ...
    if (offer.status !== 'pending') {
      throw new AppError({
        ...
        statusCode: 400,
        message: 'Cannot accept offer',
        details: { reason: `Offer is ${offer.status}` },
      });
    }
